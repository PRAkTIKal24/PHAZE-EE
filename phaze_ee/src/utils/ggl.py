...
def get_arguments():
    ...
    config_path = (
        f"phaze_ee.workspaces.{workspace_name}.{project_name}.config.{project_name}_config"
    )
    ...
    if args.mode == "new_project":
        ...
        if not os.path.exists(project_path):
            ...
            try:
                ...
                print(f"Config file not found at {config_path}. Please check your project setup.")
                sys.exit()
...
def run_training(paths: dict, config, verbose: bool = False) -> None:
    ...
    if train_loader is not None and val_loader is not None:
        history = train_early_exit_model(  # noqa: F841
            model=model,
...
def run_evaluation(paths: dict, config, verbose: bool = False) -> None:
    ...
    with torch.no_grad():
        for X, y, _ in test_loader:
            ...
            # Forward pass
            full_output, exit_outputs, _ = model(points, features, mask, return_exit_outputs=True)
    ...
    if verbose:
        ...
        for i in range(config.num_exit_points):
            ...
            print(
                f"Exit {i}: Accuracy={exit_res['accuracy']:.4f}, ROC-AUC={exit_res['roc_auc']:.4f}"
            )
